...
jest.mock("../../lib/salarySlip", () => ({
  downloadSalarySlipPdf: jest.fn(),
}));
...
async function renderModal(props) {
  ...
  await act(async () => {
    root.render(<SalarySlipModal {...props} />);
  });
  return {
    node,
    root,
...
describe("SalarySlipModal Component", () => {
  ...
  test("renders modal when open is true", async () => {
    ...
    expect(content.textContent).toContain("वेतन पर्ची डाउनलोड");
  ...
  test("allows selecting month and triggers PDF download", async () => {
    ...
    expect(salarySlipLib.downloadSalarySlipPdf).toHaveBeenCalledWith(
